#!/bin/bash
# for metagl++
if_add_programs=("1") # if 1, change metagl to metagl++
program_encoders=("bert") #  "roberta" "sbert"
...
for if_add_program in "${if_add_programs[@]}"; do
    for program_encoder in "${program_encoders[@]}"; do
                        ...
                        for loss in "${losses[@]}"; do
                            for encoder in "${encoders[@]}"; do
                                for seed in "${seeds[@]}"; do
                                    CUDA_VISIBLE_DEVICES=0 python code/run_metagl.py --seed $seed --encoder $encoder --loss $loss --limit_time $limit_time --missing_choice_ratio $missing_choice_ratio --missing_sample_ratio $missing_sample_ratio --if_calculate_difficult_level $if_calculate_difficult_level --if_add_program $if_add_program --program_encoder $program_encoder
                                done
                            done
                        done
                    done
